...
class LocalStore {
  ...
  static const _mealPlansKey = 'mealPlans';
  ...
  Future<AppData> loadAll() async {
    return AppData(
      ...
      mealPlans: _readList(_mealPlansKey, MealPlan.fromJson),
      calendarEvents: _readList(_calendarEventsKey, CalendarEvent.fromJson),
      nutritionGoals: _readList(_nutritionGoalsKey, NutritionGoal.fromJson),
      nutritionEntries: _readList(
      ...
      receipts: _readList(_receiptsKey, Receipt.fromJson),
    );
  }
  ...
  Future<void> saveAll(AppData data) async {
    await Future.wait([
      ...
      _writeList(_mealPlansKey, data.mealPlans.map((item) => item.toJson())),
      _writeList(
        _calendarEventsKey,
        data.calendarEvents.map((item) => item.toJson()),
      ),
      _writeList(
        _nutritionGoalsKey,
        data.nutritionGoals.map((item) => item.toJson()),
      ),
      _writeList(
        _nutritionEntriesKey,
      ...
      _writeList(
        _favoriteProductsKey,
        data.favoriteProducts.map((item) => item.toJson()),
      ),
      _writeList(_receiptsKey, data.receipts.map((item) => item.toJson())),
    ]);
  }
